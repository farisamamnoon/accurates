...
const QuotationForm = () => {
  ...
  const handleSubmit = async () => {
    if (validate()) {
      await generatePDF(header, items, terms);
      toast({ title: "PDF Generated", description: "Your quotation has been downloaded." });
    }
  };

  // Terms helpers
  const addTerm = () => setTerms((prev) => [...prev, { id: Date.now(), text: "" }]);
    ...
    <div className="min-h-screen bg-muted/40 p-4 md:p-8">
      <div className="mx-auto max-w-5xl rounded-xl bg-background shadow-lg border border-border overflow-hidden">
        ...
        <div className="bg-primary px-6 py-5 text-primary-foreground flex items-center gap-4">
          <img src={logoImg} alt="Company Logo" className="h-14 w-14 rounded-md bg-white/10 object-contain p-1" />
          <Input
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <InfoField label="TO" value={header.to} onChange={(v) => updateHeader("to", v)} placeholder="Company name" required error={errors.to} />
            <InfoField label="Attn" value={header.attn} onChange={(v) => updateHeader("attn", v)} placeholder="Contact person" required error={errors.attn} />
            <InfoField label="QTN" value={header.qtn} onChange={(v) => updateHeader("qtn", v)} placeholder="Qtn-1001" />
            <InfoField label="Tel" value={header.tel} onChange={(v) => updateHeader("tel", v)} placeholder="+966 XX XXX XXXX" required error={errors.tel} />
            <InfoField label="Email" value={header.email} onChange={(v) => updateHeader("email", v)} placeholder="email@example.com" type="email" required error={errors.email} />
          </div>
                ...
                {items.map((item, idx) => (
                  <tr key={item.id} className="border-t border-border hover:bg-table-highlight transition-colors">
                    ...
                    <td className="px-3 py-2">
                      <Input value={item.description} onChange={(e) => updateItem(item.id, "description", e.target.value)} placeholder="Material description"
                    ...
                    <td className="px-3 py-2">
                      <Input type="number" min={0} value={item.qty} onChange={(e) => updateItem(item.id, "qty", Number(e.target.value))}
                    ...
                    <td className="px-3 py-2">
                      <select value={item.unit} onChange={(e) => updateItem(item.id, "unit", e.target.value)}
                        ...
                        <option>Pcs</option><option>Roll</option><option>Set</option><option>Box</option><option>Kg</option><option>Ltr</option><option>Mtr</option>
                      </select>
                    </td>
                    <td className="px-3 py-2">
            ...
            <div className="p-4 space-y-2">
              {terms.map((term, idx) => (
                <div key={term.id} className="flex items-center gap-2">
                  ...
                  <Input value={term.text} onChange={(e) => updateTerm(term.id, e.target.value)} placeholder="Enter term..." className="h-8 flex-1" />
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive shrink-0"
                    onClick={() => removeTerm(term.id)} disabled={terms.length === 1}>
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
              ))}
...
const InfoField = ({ label, value, onChange, placeholder, type = "text", required, error }: {
